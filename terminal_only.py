...
def view_back(data3d, height, width):
    view = [[' ' for x in range(width)] for y in range(height)]
    for z in range(len(data3d) - 1, 0, -1):
        for y in range(len(data3d[0])):
            for x in range(len(data3d[0][0])):
                if view[y][x] == ' ':
                    view[y][x] = data3d[z][y][x]
...
functions['0 2'] = view_bottom
functions['1 2'] = view_back
...
stdscr.keypad(True)
begin_x = 0; begin_y = 0
wins = [[curses.newwin(height, width, begin_y, begin_x) for begin_x in [0,width,width * 2]] for begin_y in [0, height]]
